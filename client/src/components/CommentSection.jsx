    ...
    <div className='max-w-2xl  p-3 mx-auto w-full'>
        {
            currentUser ? (
                <div className="flex items-center gap-1 my-5 text-gray-500 text-sm">
                    ...
                    <Link to="/dashboard?tab=profile" className='text-cyan-600 text-xs hover:underline'>@{currentUser.username}</Link>
                </div>
            ):(
                <div className="text-sm text-teal-500 my-5 flex gap-1">You must be signed in to comment
                <Link className='text-blue-500 hover:underline'>Sign In</Link></div>
            )
        } {
            currentUser && (
                ...
                {
                    comments.length === 0 ? (
                        <p className='text-sm y-5'>No comments yet!</p>
                    ) : (
                       <>
